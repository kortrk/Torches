...
public class bubbleBehavior : MonoBehaviour {
	...
	void Start () {
		travel_time = Random.Range (.8f, 1.2f);
	...
	void Update () {
		if (transform.position != goal) {
			...
			if (Vector2.Distance (transform.position, goal) < screen_width * .01) 
	...
	public void setStartAndGoal(Vector2 quad_center, float width, float height, bool start_at_center){
		...
		if (by_side) {
			...
			pos_near_edge.x = quad_center.x + by_lesser * ((width * .85f) + Random.Range(0, width*.15f));
		} else {
			//x can be anything
			pos_near_edge.x = Random.Range(0, width);
			//pick a y near the top or bottom edge
			pos_near_edge.y = quad_center.y + by_lesser * ((height * .85f) + Random.Range(0, height*.15f));
		}

		if (start_at_center) {
			transform.position = Random.insideUnitCircle * (width*.2f);
			goal = pos_near_edge;
		} else {
	...
	void OnTriggerEnter(Collider other){
		//when a paddle center enters our trigger, pop
		if (other.gameObject.tag == "paddle center") {
			Pop ();
		}
	}
}
